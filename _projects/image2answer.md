...
Submission to the **Samsung Collegiate Programming Challenge (SCPC) 2025, AI
track**. It reached the finals, one of 41 out of 1,483 entrants.
...
That budget buys roughly one small VLM, and a small VLM asked to answer every
question outright is mediocre at it.
...
CLIP alone is a weak MCQ solver. Many options are semantically close, and it has
no notion of a question. But when every augmentation agrees and the margin is
wide, it is right often enough to trust, which lets it answer most of the
questions before the VLM sees anything.
...
**3. Recovering malformed answers.** A 1.7B model does not always obey "answer
